...
<span {{ $attributes->class(["form-control"])->merge(['type' => 'range']) }}>{{$attributes['value']}}</span>
@else
<input {{ $attributes->class(["form-control"])->merge(['type' => 'range']) }}>
@endif
@if(array_key_exists('key',$icon) && !empty($icon['key']))
...
@endif
@if(array_key_exists('text',$lable))
		</div>
	</div>
@endif